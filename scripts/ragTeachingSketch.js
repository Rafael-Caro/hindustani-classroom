...
function preload() {
  saptakList = loadJSON("files/ragTeaching.json");
...
function draw () {
  ...
  for (var i = 0; i < ragNames.length; i++) {
    var x = -100 + ((width + 150) / (ragNames.length + 1)) * (i+1);
...
function CreateSvara (svara, x) {
  this.x1 = x;
  this.y = map(svara.cent, 0, 1200, cursorBottom, cursorTop);
  ...
  this.displaySvara = function () {
    ...
    stroke(frontColor);
    strokeWeight(this.txtStroke);
    textSize(svaraRadius1*0.9);//this.radius*0.9);
    textStyle(BOLD);//this.txtStyle);
    fill(this.txtCol);
    text(this.name, this.x2 + svaraRadius1 * 0.4, this.y+this.radius*0.1);
    // stroke(frontColor);
    // strokeWeight(3);
    // fill(backColor);
    // textSize(svaraRadius1*0.7);
    // textStyle(NORMAL);
    // text(this.key, this.x2, this.y + (svaraRadius1*0.9)/2)
  }
}

function createSound (svara) {
  this.pitch = sa * Math.pow(2, (svara.cent/1200.0));
  this.key = svara.key;
...
function changeStep () {
  ...
  soundList = [];
  if (step == 0) {
    saptak = saptakList["saptak1"];
    for (var i = 0; i < saptak.length; i++) {
        var svara = new CreateSvara(saptak[i], width/2);
        svaraList.push(svara);
        createSound(saptak[i]);
      }
  } else if (step == 1) {
    saptak = saptakList["saptak2"];
    for (var i = 0; i < saptak.length; i++) {
        var svara = new CreateSvara(saptak[i], width/2);
        svaraList.push(svara);
        createSound(saptak[i]);
    ...
    saptak = saptakList["set1"];
    for (var i = 0; i < saptak.length; i++) {
      var x = -100 + ((width + 150) / (saptak.length + 1)) * (i + 1);
      ragNames.push(saptak[i].name);
      for (var j = 0; j < saptak[i].pitchSpace.length; j++) {
        var svara = new CreateSvara(saptak[i].pitchSpace[j], x);
        svaraList.push(svara);
        createSound(saptak[i].pitchSpace[j]);
      }
    }
  } else if (step == 4) {
    saptak = saptakList["set2"];
    for (var i = 0; i < saptak.length; i++) {
      var x = -100 + ((width + 150) / (saptak.length + 1)) * (i + 1);
      ragNames.push(saptak[i].name);
      for (var j = 0; j < saptak[i].pitchSpace.length; j++) {
