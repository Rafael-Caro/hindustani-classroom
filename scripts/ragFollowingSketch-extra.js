...
function draw () {
  ...
  if (loaded) {
    ...
    if (p != "s" && p >= minHz && p <= maxHz && showCursor.checked()) {
      ...
      stroke(frontColor);
      strokeWeight(1);
      ellipse(melCursorX, cursorY, melCurRad, melCurRad);
    }
  }
...
function start () {
  ...
  pitchTrack = loadJSON('../files/pitchTracks/'+recordingsList[selectMenu.value()].mbid+'_pitchTrack-extra.json');
